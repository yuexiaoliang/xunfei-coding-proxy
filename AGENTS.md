...
讯飞 Coding Plan API 代理 + 监控仪表盘。proxy 零依赖（纯内置模块），monitor 用 express + Vue 3 + Element Plus + Chart.js。
...
### proxy.js
- **零依赖**：只用 `http`/`https`/`url` 内置模块（流式重试需精细控制，不适合换库），HTTPS 连接池复用 TLS
...
- **统计口径**：
  ...
  - `byModel` = 按模型维度统计（总数/成功/失败/重试次数/P50耗时/平均重试），model 从请求体提取
  - `byPath` = 按上游接口路径统计（透明转发，本地 path = 上游 path）
...
- 中文沟通
- proxy 零依赖；monitor 用 express（HTTP 路由）+ Vue 3 + Element Plus（前端 UI）+ Chart.js（图表），减少重复造轮子
- 重视移动端体验（390px 无横向溢出）
- 关注数据准确性（会质疑指标合理性）
